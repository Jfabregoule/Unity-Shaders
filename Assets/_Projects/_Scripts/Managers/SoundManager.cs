...
public class SoundManager : Singleton<SoundManager>
{
    ...
    {
        foreach (SoundFX sound in _SFXList)
        {
           if (sound.key == key)
            {
    ...
    private List<AudioClip> GetAllSFXByKey(string key)
    {
    ...
    public void PlaySoundFXClip(AudioClip audioClip, Vector3 spawnPosition, float volume = 1.0f)
    {
    ...
    public void PlaySoundFXClip(AudioClip audioClip, Vector3 spawnPosition, float volume = 1.0f, float pitch = 1.0f)
    {
        AudioSource audioSource = CreateSoundFXSource(spawnPosition);
        audioSource.clip = audioClip;
        audioSource.volume = volume;
    ...
    public void PlaySoundFXClipByKey(string key, Vector3 spawnPosition, float volume = 1.0f, float pitch = 1.0f)
    {
        AudioClip audioClip = GetSFXByKey(key);
        if (audioClip != null)
        {
            PlaySoundFXClip(audioClip, spawnPosition, volume, pitch);
        }
    }

    public void PlayAllSoundFXClipsByKey(string key, Vector3 spawnPosition, float volume = 1.0f, float pitch = 1.0f)
    {
        List<AudioClip> audioClips = GetAllSFXByKey(key);
    ...
    public IEnumerator FadeInAudio(AudioSource audioSource, float duration)
    {
        ...
        {
            currentTime += Time.deltaTime;
            audioSource.volume = Mathf.Lerp(0.0f, target, currentTime / duration);
            yield return null;
        }
